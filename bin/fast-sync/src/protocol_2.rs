...
use crate::{census::Census, discovery::Discovery, types::FindContentResult};
...
pub struct Protocol<TContentKey, TMetric> {
    ...
    pub census: Arc<Census<TContentKey, TMetric>>,
    ...
    TMetric: 'static + Send + Sync + Metric,
{
    pub async fn spawn(
        ...
        let network = Arc::new(Self::new(subnetwork, discovery, utp_socket, &config));
        Self::start(&network, &config);

        network
            .census
            .init(&config.bootnodes, network.as_ref())
            .await?;
        network.census.start(network.clone());

        Ok(network)
    }

    pub fn start(service: &Arc<Self>, config: &ProtocolConfig) {
    ...
    pub fn new(
        ...
        Self {
            ...
            discovery,
            utp_socket,
            census: Arc::new(Census::new(config.census_concurrency)),
            outgoing_semaphore: Arc::new(Semaphore::new(config.outgoing_talk_request_capacity)),

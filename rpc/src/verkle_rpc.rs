...
use ethportal_api::{
    types::{
        ...
        verkle::{ContentInfo, PaginateLocalContentInfo, TraceContentInfo},
    },
    RoutingTableInfo, VerkleContentKey, VerkleContentValue, VerkleNetworkApiServer,
};
...
pub struct VerkleNetworkApi {
    network: mpsc::UnboundedSender<VerkleJsonRpcRequest>,
}

impl VerkleNetworkApi {
    pub fn new(network: mpsc::UnboundedSender<VerkleJsonRpcRequest>) -> Self {
        Self { network }
    }

    pub async fn proxy_query_to_verkle_subnet(
        &self,
        endpoint: VerkleEndpoint,
    ) -> Result<Value, RpcServeError> {
        let (resp_tx, mut resp_rx) = mpsc::unbounded_channel::<Result<Value, String>>();
        let message = VerkleJsonRpcRequest {
            endpoint,
            resp: resp_tx,
        };
        ...
        match resp_rx.recv().await {
            ...
            None => Err(RpcServeError::Message(
                "Internal error: No response from chain verkle subnetwork".to_string(),
            )),
        }
    }
}
...
impl VerkleNetworkApiServer for VerkleNetworkApi {
    ...
    async fn add_enr(&self, enr: Enr) -> RpcResult<bool> {
        let endpoint = VerkleEndpoint::AddEnr(enr);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
    ...
    async fn get_enr(&self, node_id: NodeId) -> RpcResult<Enr> {
        let endpoint = VerkleEndpoint::GetEnr(node_id);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
    ...
    async fn delete_enr(&self, node_id: NodeId) -> RpcResult<bool> {
        let endpoint = VerkleEndpoint::DeleteEnr(node_id);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
        let result: bool = from_value(result)?;
        Ok(result)
    }
    ...
    async fn lookup_enr(&self, node_id: NodeId) -> RpcResult<Enr> {
        let endpoint = VerkleEndpoint::LookupEnr(node_id);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
        let result: Enr = from_value(result)?;
        Ok(result)
    }
    ...
    async fn ping(&self, enr: Enr) -> RpcResult<PongInfo> {
        let endpoint = VerkleEndpoint::Ping(enr);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
    ...
    async fn find_nodes(&self, enr: Enr, distances: Vec<u16>) -> RpcResult<FindNodesInfo> {
        let endpoint = VerkleEndpoint::FindNodes(enr, distances);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
    ...
    async fn recursive_find_nodes(&self, node_id: NodeId) -> RpcResult<Vec<Enr>> {
        let endpoint = VerkleEndpoint::RecursiveFindNodes(node_id);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
    ...
    async fn radius(&self) -> RpcResult<DataRadius> {
        let endpoint = VerkleEndpoint::DataRadius;
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
    ...
    async fn find_content(
        ...
        let endpoint = VerkleEndpoint::FindContent(enr, content_key);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
    ...
    async fn recursive_find_content(
        ...
        let endpoint = VerkleEndpoint::RecursiveFindContent(content_key);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
        let result: ContentInfo = from_value(result)?;
        Ok(result)
    }
    ...
    async fn trace_recursive_find_content(
        ...
        let endpoint = VerkleEndpoint::TraceRecursiveFindContent(content_key);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
    ...
    async fn paginate_local_content_keys(
        ...
        let endpoint = VerkleEndpoint::PaginateLocalContentKeys(offset, limit);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
    ...
    async fn gossip(
        ...
        let endpoint = VerkleEndpoint::Gossip(content_key, content_value);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
    ...
    async fn trace_gossip(
        ...
        let endpoint = VerkleEndpoint::TraceGossip(content_key, content_value);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
    ...
    async fn store(
        ...
        let endpoint = VerkleEndpoint::Store(content_key, content_value);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
        let result: bool = from_value(result)?;
        Ok(result)
    }
    ...
    async fn local_content(&self, content_key: VerkleContentKey) -> RpcResult<VerkleContentValue> {
        let endpoint = VerkleEndpoint::LocalContent(content_key);
        let result = self.proxy_query_to_verkle_subnet(endpoint).await?;
        Ok(from_value(result)?)
    }
}

impl std::fmt::Debug for VerkleNetworkApi {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("VerkleNetworkApi").finish_non_exhaustive()
    }
}

...
impl ssz::Encode for BlockHeaderProof {
    ...
    fn ssz_append(&self, buf: &mut Vec<u8>) {
        match self {
            BlockHeaderProof::HistoricalHashesAccumulatorProof(proof) => proof.ssz_append(buf),
            BlockHeaderProof::HistoricalRootsBlockProof(proof) => proof.ssz_append(buf),
            BlockHeaderProof::HistoricalSummariesBlockProof(proof) => proof.ssz_append(buf),
        }
    }
...
/// The old block header type. Remove as part of the pr/1666 cleanup.
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode, Deserialize)]
pub struct OldHeaderWithProof {
    #[ssz(with = "ssz_header")]
    pub header: Header,
    pub proof: OldBlockHeaderProof,
}

// Remove as part of the pr/1666 cleanup.
#[derive(Debug, Clone, PartialEq, Eq, Decode, Encode, Deserialize)]
#[ssz(enum_behaviour = "union")]
pub enum OldBlockHeaderProof {
    None(SszNone),
    HistoricalHashesAccumulatorProof(HistoricalHashesAccumulatorProof),
    HistoricalRootsBlockProof(HistoricalRootsBlockProof),
    HistoricalSummariesBlockProof(HistoricalSummariesBlockProof),
...
#[derive(Clone, Debug, Default, PartialEq, Eq, Encode, Decode, Deserialize)]
pub struct SszNone {}
...
mod tests {
    ...
    fn decode_encode_headers_with_proof() {
        let file = fs::read_to_string(
            "../../portal-spec-tests/tests/mainnet/history/headers_with_proof/1000001-1000010.json",
        )
        .unwrap();
